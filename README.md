...
프로세스 목록을 확인한 결과 현재 사용자와 다른 사용자 소유의 OrbStack 관련 프로세스가 남아 있는 상태를 확인했습니다. 즉 단순히 프로그램이 설치되지 않은 것이 아니라, 기존 프로세스의 소유권과 실행 상태가 현재 사용자 환경과 충돌하는 문제로 판단했습니다.

**조치**
- 기존 OrbStack 관련 프로세스 상태를 정리했습니다.
- 현재 사용자 환경에서 OrbStack을 다시 실행했습니다.
- 마지막으로 아래 명령으로 Docker Engine 연결이 정상인지 다시 확인했습니다.
...
- 권한 오류가 발생하면 단순 재설치 전에 어떤 사용자 소유의 프로세스가 실행 중인지 확인할 필요가 있습니다.
